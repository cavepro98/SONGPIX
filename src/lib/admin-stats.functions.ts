...
export const getAdminStats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: profiles }, { data: rooms }, { data: items30 }, { data: usersList }] =
      await Promise.all([
        supabaseAdmin.from("profiles").select("id, created_at"),
        supabaseAdmin.from("rooms").select("id, owner_id, is_open, created_at"),
        supabaseAdmin
          .from("queue_items")
    ...
    const boostsToday = itemsToday.filter((i: any) => (i.paid_amount_cents ?? 0) > 0);
    const volumeTodayCents = boostsToday.reduce(
      (s: number, i: any) => s + (i.paid_amount_cents || 0),
      0,
    );
    const commissionTodayCents = Math.round(volumeTodayCents * COMMISSION_RATE);
    const ticketTodayCents =
      boostsToday.length > 0 ? Math.round(volumeTodayCents / boostsToday.length) : 0;
    ...
    const dailyMap = new Map<
      string,
      { date: string; receita: number; boosts: number; cadastros: number }
    >();
    for (let i = 29; i >= 0; i--) {
      const d = new Date(now.getTime() - i * 86400000);
      d.setHours(0, 0, 0, 0);
      const key = d.toISOString().slice(0, 10);
      dailyMap.set(key, { date: key, receita: 0, boosts: 0, cadastros: 0 });
    }
    for (const i of items) {
      const key = (i.created_at as string).slice(0, 10);
      const row = dailyMap.get(key);
      if (row && (i.paid_amount_cents ?? 0) > 0) {
        row.receita += i.paid_amount_cents;
        row.boosts += 1;
      }
    }
    ...
    const daily = Array.from(dailyMap.values()).map((r) => ({
      ...
      comissao: (r.receita * COMMISSION_RATE) / 100,
    ...
    const revenue30Cents = items.reduce((s: number, i: any) => s + (i.paid_amount_cents || 0), 0);

    // Top salas por receita (30d)
    const roomRevenue = new Map<string, number>();
    const roomBoosts = new Map<string, number>();
    for (const it of items) {
      const v = it.paid_amount_cents || 0;
      if (v <= 0) continue;
      roomRevenue.set(it.room_id, (roomRevenue.get(it.room_id) || 0) + v);
      roomBoosts.set(it.room_id, (roomBoosts.get(it.room_id) || 0) + 1);
    }
